...
class Crawler {
  ...
  async crawl() {
    ...
    console.log(
      `Crawling complete. Found ${this.externalUrls.size} external links.`
    );
    ...
    try {
      ...
      for (const url of urls) {
        if (!this.visitedUrls.has(url)) {
          ...
          await this.crawlUrl(
            url,
            this.siteTree.children[this.siteTree.children.length - 1],
            2
          );
        }
      }
  ...
  async findSitemap() {
    try {
      ...
      console.error(
        `Could not fetch robots.txt or find sitemap: ${error.message}`
      );
